...
Now the spec names its own entry point:
    ...
    "pipeline": {"stages_from": "trainer.stages:STAGES", "stages": [...]}   → pod_harness
    "exec":     {"module": "...", "args": [...]}                            → raw escape hatch

and this module launches `pod_harness.execute_job`, which imports whatever registry the
...
from pod_harness.events import EventLog, job_dir
...
# --------------------------------------------------------------------------------------
# spec handling — delegated to pod_harness so the contract has ONE definition
# --------------------------------------------------------------------------------------

def _spec_mod():
    """Import the spec contract from the engine.

    Imported lazily and tolerantly: the harness test image ships without pod_harness, and
    the API must still start there — it simply cannot validate specs as deeply.
    """
    try:
        from pod_harness import spec as spec_mod      # type: ignore
        return spec_mod
    except Exception:
        return None
    ...
    if sm is None:
        if not (raw.get("pipeline") or raw.get("exec")):
            return raw, ["spec needs 'pipeline' or 'exec' "
                         "(pod_harness not installed here, so validation is minimal)"]
        return raw, []
    try:
...
def validate(spec: dict, *, cr: code_mod.CodeRoot | None = None) -> dict:
    ...
    if not result["problems"] and spec.get("pipeline"):
        try:
            ...
            from pod_harness import execute_job as EJ       # type: ignore
...
def _command(spec: dict, spec_path: Path, job_id: str, cr: code_mod.CodeRoot) -> list[str]:
    ...
    return [sys.executable, "-u", "-m", "pod_harness.execute_job",
